...
synthdata is a small package for generating synthetic tabular data using a Gaussian copula, with a set of evaluation metrics (KS/Wasserstein, correlation differences, PRDC, StatisticalSimScore via `pymdma` and more).
...
- For numerical stability a small regularization term is added to the diagonal of the estimated correlation matrix; for a minimal-distortion nearest-PD solution consider implementing Higham’s algorithm.
